...
// Fire-and-forget event send. Inngest only drives background work, so a
...
export const sendEvent = async (payload) => {
  try {
    ...
    console.warn(
      `[inngest] event "${payload?.name}" not sent: ${error.message}` +
        (/event key/i.test(error.message)
          ? " - set INNGEST_EVENT_KEY in server/.env, or run `npx inngest-cli dev` for local development"
          : "")
    );
    return false;
  }
  ...
  async ({ event, step }) => {
    ...
    await sendEmail({
      ...
      body: `<div style="font-family: Arial, sans-serif; line-height: 1.5;">
        ...
        <p>Enjoy the show! 🍿</p>
        <p>Thanks for booking with us!<br />- MovieSeat Team</P>
      </div>`,
    });
  }
);
  ...
  async ({ step }) => {
    ...
    const results = await step.run("send-all-reminders", async () => {
      return await Promise.allSettled(
        ...
        <br />
        <p>Enjoy the show! 🍿 - MovieSeat Team</P>
      </div>`,
          })
        )
      );
  ...
  async ({ event }) => {
    ...
    for (const user of users) {
      ...
      const body = `<div style="font-family: Arial, sans-serif; padding: 20px;">
        ...
        <p>Visit our website - <a href="https://showtime-eta.vercel.app/">MovieSeat</a> 🔗</p>
        <br />
        <p>Thanks, <br />MovieSeat Team</p>
      </div>`;
